...
@router.get("/documents/audit-logs")
async def stub_doc_audit(
    response: Response,
    _: Annotated[AuthenticatedUser, Depends(get_current_user)],
) -> list[Any]:
    return _empty_list(response)
...
@router.get("/documents/overdue")
async def stub_doc_overdue(
    response: Response,
    _: Annotated[AuthenticatedUser, Depends(get_current_user)],
) -> list[Any]:
    return _empty_list(response)
...
async def stub_doc_requirements(
    response: Response,
    _: Annotated[AuthenticatedUser, Depends(get_current_user)],
) -> list[Any]:
    return _empty_list(response)
...
@router.post("/documents/archive-run")
async def stub_doc_archive_run(
    response: Response,
    _: Annotated[AuthenticatedUser, Depends(get_current_user)],
) -> dict[str, Any]:
    return _stub_action(response)
...
@router.post("/documents/purge-archives")
async def stub_doc_purge_archives(
    response: Response,
    _: Annotated[AuthenticatedUser, Depends(get_current_user)],
) -> dict[str, Any]:
    return _stub_action(response)
