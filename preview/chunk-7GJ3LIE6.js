...
function resolveErrorMessage(error) {
  ...
  if (error.status === 0) {
    return "API indisponible. V\xE9rifiez la connexion au backend.";
  }
...
var AuthService = class _AuthService {
  ...
  async loginWithEmail(email, password) {
    ...
    try {
      ...
      const response = await firstValueFrom(this.http.post(this.buildUrl(API_ENDPOINTS.auth.login), { username: email, password }, { context }));
      const token = this.extractAccessToken(response);
      if (!token) {
        throw new Error("Token absent dans la r\xE9ponse de login");
      }
  ...
  refreshToken() {
    ...
    const context = new HttpContext().set(SKIP_ERROR_TOAST, true);
    return firstValueFrom(this.http.post(this.buildUrl(API_ENDPOINTS.auth.refresh), { refreshToken }, { context })).then((response) => {
      const token = this.extractAccessToken(response);
      if (!token) {
        throw new Error("Token absent dans la r\xE9ponse de refresh");
      }
...
@angular/animations/fesm2022/animations.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-7GJ3LIE6.js.map
